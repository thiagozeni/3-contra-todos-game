...
await sleep(1200)
for (const [i,name] of [[0,'werdum'],[1,'dida'],[2,'thor']]) {
  await p.evaluate((idx)=>{window.__game.scene.getScene('SelectScene').selectChar(idx)}, i)
  await sleep(400)
  await p.screenshot({path:`/tmp/loop-shots/stat-${name}.png`, clip:{x:80,y:670,width:470,height:380}})
}
console.log('done')
await b.close()
